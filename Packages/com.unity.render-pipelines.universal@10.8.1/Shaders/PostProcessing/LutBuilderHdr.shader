Shader "Hidden/Universal Render Pipeline/LutBuilderHdr"
{
    HLSLINCLUDE
        ...
        #pragma multi_compile_local _ _TONEMAP_UNITY_ACES _TONEMAP_NEUTRAL _TONEMAP_UE4_FILMIC _TONEMAP_ACES_FILMIC
        ...
        float3 Tonemap(float3 colorLinear)
        {
            ...
            #elif _TONEMAP_UNITY_ACES
            {
                // Note: input is actually ACEScg (AP1 w/ linear encoding)
                float3 aces = ACEScg_to_ACES(colorLinear);
                colorLinear = AcesTonemap(aces);
            }
            #elif _TONEMAP_UE4_FILMIC
            {
                colorLinear = FilmicTonemap(colorLinear);
            }
            #elif _TONEMAP_ACES_FILMIC
                colorLinear = StephenHillACES(colorLinear);
            #endif
