...
float _FilmSlope;// default = 0.88;
float _FilmToe;// default = 0.55;
float _FilmShoulder;// default = 0.26;
float _FilmBlackClip;// default = 0.0;
float _FilmWhiteClip;// default = 0.04;
...
float3 FilmicTonemap(float3 linearColor)
{
    ...
    float3 ColorAP1 = unity_to_ACEScg(linearColor);
    ...
    //     float ExpandAmount = ( 1 - exp2( -4 * ChromaDistSqr ) ) * ( 1 - exp2( -4 * LumaAP1*LumaAP1 ) );
    //
    ...
    // Blue correction
    ColorAP1 = mul( BlueCorrectAP1, ColorAP1 );
    ...
    // BlueCorrection
    ToneColor = mul( BlueCorrectInvAP1, ToneColor );

    ToneColor = lerp(dot(ToneColor, AP1_RGB2Y).xxx, ToneColor, ODT_SAT_FACTOR.xxx);
