#ifndef UNIVERSAL_POSTPROCESSING_EDGE_DETECTION_INCLUDED
#define UNIVERSAL_POSTPROCESSING_EDGE_DETECTION_INCLUDED
...
{
    return 1;// SAMPLE_TEXTURE2D_X(_Texture, sampler_Texture, UnityStereoTransformScreenSpaceTex(uv)).r;
}
...
float4 SampleNeighborhood(float2 uv, float thickness) {
    ...
    float2 delta = _SourceTex_TexelSize.xy * thickness;
    float4 sum = 0;
    float weight = 0;
    for (int i = 0; i < 8; i++) {
...
float3 EdgeDetectionFragmentProgram(float3 color, float2 uv)
{
    ...
    color = lerp(color, _EdgeColor.rgb, edge * _Intensity);
    return color;
}
#endif
