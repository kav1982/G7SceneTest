Shader "Hidden/Universal Render Pipeline/UberPost"
{
    HLSLINCLUDE
        ...
        #pragma multi_compile_local_fragment _ FOGOFWAR
        #pragma multi_compile_local_fragment _ FOGOFWAR_DEPTH
        #pragma multi_compile_local_fragment _ EDGE_DETECTION
        #pragma multi_compile_local_fragment _ MY_SATURATION
        ...
        #include "Packages/com.unity.render-pipelines.universal/Shaders/PostProcessing/FogOfWar.hlsl"
        #include "Packages/com.unity.render-pipelines.universal/Shaders/PostProcessing/EdgeDetection.hlsl"
        ...
        half4 Frag(Varyings input) : SV_Target
        {
            ...
            {
                #if _TAA
                    float2 offset = _TAA_TempTexture_TexelSize.xy;
                    half4 u = SAMPLE_TEXTURE2D(_TAA_TempTexture, sampler_LinearClamp, uvDistorted + float2(0, offset.y));
                    half4 d = SAMPLE_TEXTURE2D(_TAA_TempTexture, sampler_LinearClamp, uvDistorted + float2(0, -offset.y));
                    half4 l = SAMPLE_TEXTURE2D(_TAA_TempTexture, sampler_LinearClamp, uvDistorted + float2(-offset.x, 0));
                    half4 r = SAMPLE_TEXTURE2D(_TAA_TempTexture, sampler_LinearClamp, uvDistorted + float2(offset.x, 0));
                    half4 c = SAMPLE_TEXTURE2D(_TAA_TempTexture, sampler_LinearClamp, uvDistorted);
                    half4 sharp = c - (u + d + l + r + c) * 0.2;
                    c += sharp * _TAA_Sharpness;
                #else
            ...
            #if EDGE_DETECTION
                color = EdgeDetectionFragmentProgram(color, uv);
            #endif
            ...
            #if defined(FOGOFWAR)  // 迷雾
            half4 fogTex = RenderFogOfWar(uv);
            color.rgb = lerp(fogTex.rgb, color.rgb, fogTex.a);
            #endif  // 迷雾
            ...
            #if MY_SATURATION//饱和度调整
            {
                half gray = 0.2125 * color.r + 0.7154 * color.g + 0.0721 * color.b;
                //屏幕下半部分
                half _sat = step(uvDistorted.y, _MySaturationArg.y) * lerp(1, _MySaturationArg.z, (_MySaturationArg.y - uvDistorted.y) / _MySaturationArg.y);
                //屏幕上半部分
                _sat += step(_MySaturationArg.y, uvDistorted.y) * lerp(1, _MySaturationArg.x, (uvDistorted.y - _MySaturationArg.y) / (1 - _MySaturationArg.y));
                color = lerp(half3(gray, gray, gray), color, _sat);
            }
            #endif
