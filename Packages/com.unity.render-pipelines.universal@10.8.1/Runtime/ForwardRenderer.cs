...
namespace UnityEngine.Rendering.Universal
{
    ...
    public sealed class ForwardRenderer : ScriptableRenderer
    {
...
#endif
        CopyColorPass m_FinalCopyPass;
        UIMergePass m_UIMergePass;

        RenderTargetHandle m_ActiveCameraColorAttachment;
        RenderTargetHandle m_ActiveCameraDepthAttachment;
        RenderTargetHandle m_CameraColorAttachment;
        RenderTargetHandle m_CameraDepthAttachment;
        RenderTargetHandle m_GameCameraColorAttachment;
        RenderTargetHandle m_GameCameraDepthAttachment;
        RenderTargetHandle m_UICameraColorAttachment;
        RenderTargetHandle m_UICameraDepthAttachment;
        ...
        RenderTargetHandle m_ChangeColorAttachment;
...
#pragma warning restore 414
        ...
        Material m_UIMergeMaterial;

        bool m_hasGameCamera = false;
        bool m_hasUICamera = false;
        bool m_hasMerge = false;
        ...
        public ForwardRenderer(ForwardRendererData data) : base(data)
        {
            ...
            m_UIMergeMaterial = CoreUtils.CreateEngineMaterial(data.shaders.uiMergePS);
            ...
            m_FinalCopyPass = new CopyColorPass(RenderPassEvent.AfterRenderingPostProcessing + 3, m_SamplingMaterial, m_BlitMaterial);
            m_UIMergePass = new UIMergePass(RenderPassEvent.AfterRenderingPostProcessing + 2, m_UIMergeMaterial, m_BlitMaterial);
            ...
            m_GameCameraColorAttachment.Init("_GameCameraColorTexture");
            m_GameCameraDepthAttachment.Init("_GameCameraDepthAttachment");
            m_UICameraColorAttachment.Init("_UICameraColorTexture");
            m_UICameraDepthAttachment.Init("_UICameraDepthAttachment");
            ...
            m_ChangeColorAttachment.Init("_ChangeColorTexture");
        ...
        public override void Setup(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            RenderTextureDescriptor cameraTargetDescriptor = cameraData.isUICamera ? cameraData.uiCameraTargetDescriptor : cameraData.gameCameraTargetDescriptor;
            ...
            if (cameraData.renderType == CameraRenderType.Base)
            {
                m_hasMerge = false;
                m_hasUICamera = false;
                m_hasGameCamera = !cameraData.isUICamera;
            }

            if (renderingData.skipAllPass)
            {
                m_ActiveCameraColorAttachment = m_CameraColorAttachment;
                m_ActiveCameraDepthAttachment = m_CameraDepthAttachment;
                CreateCameraRenderTarget(context, ref cameraTargetDescriptor, true, true);
            ...
            {
                m_ActiveCameraColorAttachment = m_hasMerge ? m_CameraColorAttachment : (cameraData.isUICamera ? m_UICameraColorAttachment : m_GameCameraColorAttachment);
            ...
            //非UI相机的输出都输出到_GameCameraColorTexture
            if (cameraData.renderType == CameraRenderType.Base)
            {
                RenderTargetHandle cameraTargetHandle = RenderTargetHandle.GetCameraTarget(cameraData.xr);

                m_ActiveCameraColorAttachment = (createColorTexture) ? m_GameCameraColorAttachment : cameraTargetHandle;
                m_ActiveCameraDepthAttachment = (createDepthTexture) ? m_GameCameraDepthAttachment : cameraTargetHandle;
            ...
            {
                //已经合并完UI之后的渲染都直接渲染到m_CameraColorAttachment
                m_ActiveCameraColorAttachment = m_hasMerge ? m_CameraColorAttachment : m_GameCameraColorAttachment;
                m_ActiveCameraDepthAttachment = m_hasMerge ? m_CameraDepthAttachment : m_GameCameraDepthAttachment;
                //if (m_hasMerge) SetNotFirstTime();
            }

            if (cameraData.isUICamera)
            {
                m_hasUICamera = true;
                m_ActiveCameraColorAttachment = m_UICameraColorAttachment;
                m_ActiveCameraDepthAttachment = m_UICameraDepthAttachment;
                CreateCameraRenderTarget(context, ref cameraTargetDescriptor, true, true);

                CreateCameraRenderTarget(context, ref cameraData.cameraTargetDescriptor);

                //因为UI渲染的结果经常都不是全屏不透明的，所以在渲染新的之前要先清理一下输出
                CommandBuffer cmd = CommandBufferPool.Get();
                cmd.Clear();
                cmd.SetRenderTarget(m_UICameraColorAttachment.Identifier(), RenderBufferLoadAction.DontCare, RenderBufferStoreAction.Store, m_UICameraDepthAttachment.Identifier(), RenderBufferLoadAction.DontCare, RenderBufferStoreAction.DontCare);
                ...
                //CommandBuffer cmd = CommandBufferPool.Get();
                //cmd.Clear();
                //using (new ProfilingScope(cmd, changeForUI))
                //{
                //    context.ExecuteCommandBuffer(cmd);
                //    cmd.Clear();
                //    cmd.GetTemporaryRT(m_ChangeColorAttachment.id, cameraTargetDescriptor);
                //    cmd.SetGlobalTexture(ShaderPropertyId.sourceTex, m_CameraColorAttachment.Identifier());
                //    cmd.EnableShaderKeyword(ShaderKeywordStrings.LinearToSRGBConversion);
                //    cmd.DisableShaderKeyword(ShaderKeywordStrings.SRGBToLinearConversion);
                //    cmd.SetRenderTarget(m_ChangeColorAttachment.Identifier(), RenderBufferLoadAction.DontCare, RenderBufferStoreAction.Store, RenderBufferLoadAction.DontCare, RenderBufferStoreAction.DontCare);
                //    cmd.Blit(m_CameraColorAttachment.Identifier(), m_ChangeColorAttachment.Identifier(), m_BlitMaterial);
                //    if (cameraData.needChangeForUI)
                //    {
                //        context.ExecuteCommandBuffer(cmd);
                //        CreateCameraRenderTarget(context, ref cameraTargetDescriptor, true, true, true);
                //        cmd.Clear();
                //    }
                //    cmd.SetGlobalTexture(ShaderPropertyId.sourceTex, m_ChangeColorAttachment.Identifier());
                //    cmd.DisableShaderKeyword(ShaderKeywordStrings.LinearToSRGBConversion);
                //    cmd.SetRenderTarget(m_CameraColorAttachment.Identifier(), RenderBufferLoadAction.DontCare, RenderBufferStoreAction.Store, RenderBufferLoadAction.DontCare, RenderBufferStoreAction.DontCare);
                //    cmd.Blit(m_ChangeColorAttachment.Identifier(), m_CameraColorAttachment.Identifier(), m_BlitMaterial);
                //    cmd.ReleaseTemporaryRT(m_ChangeColorAttachment.id);
                //}
                //context.ExecuteCommandBuffer(cmd);
                //cmd.Clear();
                //CommandBufferPool.Release(cmd);
                Shader.DisableKeyword(ShaderKeywordStrings.UI_SRGB);
            }
            else
            {
                Shader.EnableKeyword(ShaderKeywordStrings.UI_SRGB);
            }
#if UNITY_EDITOR
            if (cameraData.isSceneViewCamera)
            {
                Shader.EnableKeyword(ShaderKeywordStrings.NEED_GAMMA);
            }
            else
            {
                Shader.DisableKeyword(ShaderKeywordStrings.NEED_GAMMA);
            }
#endif
            ...
            //因为在UI相机渲染之后加了UI相机和场景相机的合并处理，所以UI相机就要为true
            bool hasPassesAfterPostProcessing = cameraData.isUICamera || activeRenderPassQueue.Find(x => x.renderPassEvent == RenderPassEvent.AfterRendering) != null;
            ...
            if (lastCameraInTheStack)
            {
                ...
                if (applyPostProcessing)
                {
                    ...
                    m_PostProcessPass.Setup(cameraTargetDescriptor, m_ActiveCameraColorAttachment, destination, m_ActiveCameraDepthAttachment, m_ColorGradingLut, applyFinalPostProcessing, doSRGBConvertion, cameraData.isUICamera);
                ...
                {
                    m_FinalPostProcessPass.SetupFinalPass(sourceForFinalPass, false);// cameraData.isUICamera);
                ...
                {
                    //固定是从_CameraColorTexture输出到屏幕
                    m_FinalBlitPass.Setup(cameraTargetDescriptor, (m_hasMerge || cameraData.isUICamera) ? m_CameraColorAttachment : (m_hasGameCamera ? m_GameCameraColorAttachment : m_UICameraColorAttachment));
                    //m_FinalBlitPass.Setup(cameraTargetDescriptor, sourceForFinalPass);//, !cameraData.isUICamera && !applyFinalPostProcessing);
            ...
            {
                m_PostProcessPass.Setup(cameraTargetDescriptor, m_ActiveCameraColorAttachment, m_AfterPostProcessColor, m_ActiveCameraDepthAttachment, m_ColorGradingLut, false, false, false);// cameraData.isUICamera);
                EnqueuePass(m_PostProcessPass);
            }
#if UNITY_EDITOR
            if (cameraData.isUICamera || cameraData.isPreviewCamera || cameraData.isSceneViewCamera)
#else
            if (cameraData.isUICamera)
#endif
            {
                //如果_UICameraColorTexture和否则就直接复制_GameCameraColorTexture有内容，就合并，否则就直接复制
                if (m_hasGameCamera && m_hasUICamera)
                {
                    m_UIMergePass.Setup(m_ActiveCameraColorAttachment.Identifier(), m_CameraColorAttachment, true);
                }
                else
                {
                    //只有UI相机时，复制的同时还要转换颜色空间
                    m_UIMergePass.Setup(m_ActiveCameraColorAttachment.Identifier(), m_CameraColorAttachment, false, m_hasUICamera);
                }
                m_UIMergePass.ConfigureClear(ClearFlag.None, Color.clear);
                EnqueuePass(m_UIMergePass);
                m_hasMerge = true;
            }
            ...
            {
                Shader.EnableKeyword(ShaderKeywordStrings.EDITOR_BLUR);
                m_FinalCopyPass.Setup(m_CameraColorAttachment.Identifier(), m_ChangeColorAttachment, Downsampling.None);
                m_FinalCopyPass.ConfigureClear(ClearFlag.None, Color.clear);
                m_FinalCopyPass.SetBackCameraTarget(true);
                EnqueuePass(m_FinalCopyPass);
            }
#endif
        }
        ...
        void CreateCameraRenderTarget(ScriptableRenderContext context, ref RenderTextureDescriptor descriptor, bool createColor, bool createDepth, bool releaseOld = false)
        {
            CommandBuffer cmd = CommandBufferPool.Get();
