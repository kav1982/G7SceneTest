...
namespace UnityEngine.Rendering.Universal.Internal
{
    ...
    public class PostProcessPass : ScriptableRenderPass
    {
        ...
        public void Setup(in RenderTextureDescriptor baseDescriptor, in RenderTargetHandle source, in RenderTargetHandle destination, in RenderTargetHandle depth, in RenderTargetHandle internalLut, bool hasFinalPass, bool enableSRGBConversion, bool toLinear)
        {
            ...
            m_sRGBToLinear = toLinear;
        }

        public void SetupFinalPass(in RenderTargetHandle source, bool toLinear)
        {
            m_Source = source;
        ...
        void Render(CommandBuffer cmd, ref RenderingData renderingData)
        {
            ...
            if (m_sRGBToLinear)
            {
                ...
                RenderingUtils.Blit(cmd, GetSource(), GetDestination(), m_BlitMaterial);
            ...
            using (new ProfilingScope(cmd, ProfilingSampler.Get(URPProfileId.UberPostProcess)))
            {
                ...
                while (renderingData.postProcessingSetMat.Count > 0)
                {
                    renderingData.postProcessingSetMat.Dequeue().Invoke(m_Materials.uber);
                }
        ...
        void SetupColorGrading(CommandBuffer cmd, ref RenderingData renderingData, Material material)
        {
            ...
            else
            {
                switch (m_Tonemapping.mode.value)
                {
                    ...
                    case TonemappingMode.Unity_ACES: material.EnableKeyword(ShaderKeywordStrings.TonemapUnityACES); break;
                    case TonemappingMode.UE4_ACES:
                        material.EnableKeyword(ShaderKeywordStrings.TonemapUE4ACES);
                        material.SetFloat(ShaderConstants._FilmSlope, m_Tonemapping.filmicSlope.value);
                        material.SetFloat(ShaderConstants._FilmToe, m_Tonemapping.filmicToe.value);
                        material.SetFloat(ShaderConstants._FilmShoulder, m_Tonemapping.filmicShoulder.value);
                        material.SetFloat(ShaderConstants._FilmBlackClip, m_Tonemapping.filmicBlackClip.value);
                        material.SetFloat(ShaderConstants._FilmWhiteClip, m_Tonemapping.filmicWhiteClip.value);
                        break;
                    case TonemappingMode.ACES: material.EnableKeyword(ShaderKeywordStrings.TonemapACESFilmic); break;
                    default: break; // None
                }
            }
        }
