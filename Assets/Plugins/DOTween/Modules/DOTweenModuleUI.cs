...
#region RectTransform
        ...
        public static Tweener DoRectScale(this RectTransform target, Vector3 endValue, float duration, bool snapping = false)
        {
            TweenerCore<Vector3, Vector3, VectorOptions> t = target.DOScale(endValue, duration);
            t.SetOptions(snapping).SetTarget(target);
            return t;
        }

        public static Tweener DoAngle(this RectTransform target, float duration, float endValue, TweenCallback onComplete = null, bool snapping = false)
        {
            Tweener tweener = DOTween.To(() => target.localEulerAngles, x => target.localEulerAngles = x, new Vector3(0, 0, endValue), duration)
