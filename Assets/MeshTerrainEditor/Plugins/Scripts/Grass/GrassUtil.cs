...
namespace MTE
{
    public class GrassUtil
    {
        ...
        public static void GenerateGrassStarObject(Vector3 position, Quaternion rotation,
            float width, float height, Material material, bool isStatic,
        ...
        #region grass: custom mesh
        
        public static void GenerateGrassCustomMeshObject(Vector3 position, Quaternion rotation,
            Mesh grassMesh, Material material,
            float scaleXZ, float scaleY, bool isStatic,
            out GameObject grassGameObject, out MeshRenderer grassMeshRenderer)
        {
            GameObject obj = new GameObject(grassMesh.name + "_instance");
            var meshFilter = obj.AddComponent<MeshFilter>();
            var meshRenderer = obj.AddComponent<MeshRenderer>();

            meshRenderer.sharedMaterial = material;
            meshFilter.sharedMesh = grassMesh;
            ...
            obj.transform.position = position;
            obj.transform.rotation = rotation;
            obj.transform.localScale = new Vector3(scaleXZ, scaleY, scaleXZ);
