    ...
    internal class SplatPainter : IEditor
    {
        ...
        public float BrushDirection
        {
            ...
            set
            {
                ...
                {
                    EditorPrefs.SetFloat($"MTE_{nameof(SplatPainter)}.brushDirection", this.brushDirection);
        ...
        private void LoadSavedParamter()
        {
            ...
            brushDirection = EditorPrefs.GetFloat($"MTE_{nameof(TextureArrayPainter)}.brushFlow", DefaultBrushDirection);
        }
            ...
            if (PainterMode == EditorFilterMode.SelectedGameObject)
            {
                    ...
                    (e.type == EventType.MouseDown && e.shift == false && e.alt == false && e.button == 0))
                {
                    ...
                    for (var i = 0; i < brushSizeInTexel; i++)
                    {
                        ...
                        {
                            var maskU = (float)i / brushSizeInTexel;
                            var maskV = (float)j / brushSizeInTexel;
                            var maskUV = MathEx.Rotate(new Vector2(maskU-0.5f, maskV-0.5f), BrushDirection)
                                + new Vector2(0.5f, 0.5f);
                            BrushStrength[j * brushSizeInTexel + i] =
                                maskTexture.GetPixelBilinear(maskUV.x, maskUV.y).a;
                        }
                    }
