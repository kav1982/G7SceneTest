                    ...
                    {//adding grasses
                        ...
                        else
                        {
                            ...
                            var samples = FastPoissonDiskSampling.Sample(
                                brushBoundMin, brushBoundMax, Distance);
                            Dev.Profile.End();
                            ...
                            //remove outside-brush and overlapped samples
                            for (var i = samples.Count - 1; i >= 0; i--)
                            {
                                ...
                                var existingNumber = GrassMap.GetGrassNumberInCircle(
                                    new Vector3(p.x, 0, p.y), Distance);
                                if (existingNumber > 0)
                                {
                                    samples.RemoveAt(i);
                                }
        ...
        private void CreateGrassInstances()
        {
            ...
            for (int j = 0; j < grassPositions.Count; j++)
            {
                ...
                {//remove overlapped positions
                    if (GrassMap.Intersect(grassPosition, width))
                    {
                        continue;
                    }
                }
