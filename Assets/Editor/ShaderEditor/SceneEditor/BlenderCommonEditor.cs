...
public class BlenderCommonEditor : ShaderGUI
{
    ...
    public void FindProperties(MaterialProperty[] props)
    {
        ...
        _SET_LIGHT_COL = FindProperty("_SET_LIGHT_COL", props);
    ...
    public void ShaderPropertiesGUI(Material material)
    {
        ...
        if (_TEXTURE_COLOR_MAPING.floatValue != 0)
        {
            ...
            if (GUILayout.Button("应用修改到贴图"))
            {
                if (_BaseMap.textureValue != null)
                    if (EditorFileTools.ChangeTextureColor(AssetDatabase.GetAssetPath(_BaseMap.textureValue), _Gamma.floatValue, _Bright.floatValue, _Contrast.floatValue, _Hue.floatValue, _Saturation.floatValue, _Value.floatValue, _HSVFac.floatValue))
                    {
        ...
        EditorGUILayout.Space();
        m_MaterialEditor.ShaderProperty(_SET_LIGHT_COL, "光源颜色");
        if (_SET_LIGHT_COL.floatValue != 0)
        {
            EditorGUI.indentLevel++;
            {
        ...
        m_MaterialEditor.ShaderProperty(_SmoothDiff, "平滑NdotL");
