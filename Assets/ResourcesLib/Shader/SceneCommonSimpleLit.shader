Shader "Bioum/Scene/CommonSimple"
{
    Properties
    {
        ...
        [ToggleUI] _OutlineToggle("边缘描边开关", float) = 0
        _EdgeMaskScale("Mask强度", range(0, 10)) = 0.8
        _EdgeThred("边缘阈值", range(0, 1)) = 0.6
        _EdgePow("边缘过渡", range(0.01, 10)) = 0.5
        _EdgeAngle("角度", range(-3, 3)) = 0
        _EdgeAngleScale("角度强度", range(0, 20)) = 0
        _EdgeColor("颜色", Color) = (0.3, 0.3, 0.3, 1)
        _EdgeParam("描边参数", vector) = (0.8, 0.6, 0.5, 0)
    ...
    HLSLINCLUDE
        #include "SceneCommonSimpleLitInput.hlsl"
    ENDHLSL
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            #pragma shader_feature_local _ _TERRAIN_BLEND_FUNC
            ...
            #include "SceneCommonSimpleLitPass.hlsl"
            ENDHLSL
        }
        Pass
        {
