...
{
    float2 val = float2(_Time.w * g_WindParam.w + positionWS.xz * g_WindParam.z);
    val = val % TWO_PI;
    half4 s = sin(float4(val.xyyx));
    half2 waveXZ = s.xy * s.zw;
    return waveXZ * g_WindParam.xy;
}
half2 WindAnimation(half3 positionWS, float4 param, float falloff)
{
    float2 val = float2(_Time.w * param.w + positionWS.xz * param.z * 4);
    val = val % TWO_PI;
    half4 s = sin(float4(val.xyyx));
    half2 waveXZ = s.xy * s.zw;
    return waveXZ * param.xy * falloff;
}
...
float SimpleNoise(float2 uv, float scale)
{
    ...
    t += valuenoise(float2(uv * scale / freq)) * amp;
...
//#define SHADER_API_METAL
float2 PlantsAnimationNoise(float3 positionWS)
{
#ifdef SHADER_API_METAL
    return WindAnimation(positionWS);
#else
...
float2 PlantsAnimationNoise(float3 positionWS, half2 direction, half scale, half speed, out half2 waveColor)
{
#ifdef SHADER_API_METAL
    waveColor = 1.0;
    return WindAnimation(positionWS, half4(direction, scale, speed), 1.0);
#else
...
float2 PlantsAnimationNoise(float3 positionWS, half2 direction, half scale, half speed)
{
#ifdef SHADER_API_METAL
    return WindAnimation(positionWS, half4(direction, scale, speed), 1.0);
#else
...
float2 PlantsAnimationNoise(float3 positionWS, float4 param, half falloff)
{
#ifdef SHADER_API_METAL
    return WindAnimation(positionWS, param, falloff);
#else
