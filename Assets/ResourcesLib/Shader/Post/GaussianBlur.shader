...
Shader "Post/GaussianBlur"
{
	...
	v2f_ext vert_ext(appdata_img v)
	{
		v2f_ext o;
		o.pos = UnityObjectToClipPos(v.vertex);
		//uv坐标
		...
		o.uv01 = v.texcoord.xyxy + _extValue * float4(1, 0, -1, 0);//左右两个像素的
		o.uv23 = v.texcoord.xyxy + _extValue * float4(0, 1, 0, -1);//上下两个像素的

		return o;
	}

	fixed4 frag_ext(v2f_ext i) : SV_Target
	{
		fixed4 color = tex2D(_MainTex, i.uv);
		fixed4 c = tex2D(_MainTex, i.uv01.xy);
		color.r = max(c.r, color.r);
		color.g = min(c.g, color.g);
	...
	SubShader
	{
		...
		Pass
		{
			...
			CGPROGRAM
			#pragma vertex vert_ext
			#pragma fragment frag_ext
			ENDCG
		}
	}
